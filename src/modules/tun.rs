...
pub async fn run_tun_module(
    ...
    println!(
        "[+] TUN интерфейс '{}' успешно поднят с IP {}",
...
fn setup_routes(cfg: &ClientConfig) -> Result<(), DynError> {
    ...
    println!(
        "[*] Настройка маршрутизации через TUN '{}'...",
        cfg.tun_name
    );
    ...
    #[cfg(target_os = "linux")]
    {
        let default_gw_out = Command::new("sh")
            ...
            .output()?;
        let default_gw = String::from_utf8_lossy(&default_gw_out.stdout)
            .trim()
            .to_string();

        if !default_gw.is_empty() && !server_ip.is_empty() {
            let _ = Command::new("ip")
                .args(["route", "add", server_ip, "via", &default_gw])
                .output();
        }

        let _ = Command::new("ip")
            .args(["route", "add", "0.0.0.0/1", "dev", &cfg.tun_name])
            .output();
        let _ = Command::new("ip")
            .args(["route", "add", "128.0.0.0/1", "dev", &cfg.tun_name])
            .output();
    ...
    #[cfg(target_os = "windows")]
    {
        let _ = Command::new("route")
            ...
            .output();
        let _ = Command::new("route")
            .args(["add", "0.0.0.0", "mask", "128.0.0.0", &cfg.tun_gateway])
            .output();
        let _ = Command::new("route")
            .args(["add", "128.0.0.0", "mask", "128.0.0.0", &cfg.tun_gateway])
            .output();
